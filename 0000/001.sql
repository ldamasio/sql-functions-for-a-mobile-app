...
Obs: a abordagem do get_hierarchy_level(pc.id) >= (get_hierarchy_level(sd.id) - 1)
apenas substrai 1 da conta analítica, por isso a função estava retornando apenas 1 
nível da conta analítica.
    ...
    saldos AS (
        ...
        FROM
            sigilomobile.retorna_saldo_e_movimentacao_contas(
                1, 
                '1.1.1.1.,1.1.1.2.,1.1.1.3.,1.1.1.4.,1.1.2.,2.1.', 
                '2024-04-30', 
                '2024-04-30'
            ) sd
        ORDER BY
    ...
    FROM
        ...
        INNER JOIN saldos sd
            ON (sd.id ILIKE pc.id || '%')
    WHERE
	pc.id ILIKE '1.1.1.1.%' OR 
	pc.id ILIKE '1.1.1.2.%' OR 
	pc.id ILIKE '1.1.1.3.%' OR
	pc.id ILIKE '1.1.1.4.%' OR
	pc.id ILIKE '1.1.2.%' OR
	pc.id ILIKE '2.1.%'
    GROUP BY
        sd.cod_empresa,
        pc.id,
